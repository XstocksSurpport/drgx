...
export function parseWalletImport(raw) {
  ...
  for (const line of lines) {
    const addrLine = line.match(/^Address:\s*(zs1[a-z0-9]+)/i);
    if (addrLine) {
      address = addrLine[1];
      break;
    }
...
function pickAddress(...candidates) {
  for (const value of candidates) {
    const match = String(value || '').trim().match(ZADDR);
...
export async function resolveWalletAddress(type, rawSecret, rpcCall, clientAddress = '') {
  const { address: parsedAddr, secret } = parseWalletImport(rawSecret);
  const address = pickAddress(parsedAddr, clientAddress);
  if (address) return { address, secret };

  if (secret.startsWith('secret-extended-key') || /^[A-Za-z0-9]{60,}$/.test(secret)) {
    try {
      const imported = await rpcCall('z_importkey', [secret, 'no']);
      if (typeof imported === 'string' && imported.startsWith('zs')) {
      ...
      throw new Error(
        msg.includes('Connection') || msg.includes('ECONNREFUSED') || msg.includes('fetch failed')
          ? '无法自动解析地址。请在下方填写 zs1 地址，或粘贴 ObsidianDragon 完整导出'
  ...
  throw new Error('无法自动解析地址。请在下方填写 zs1 地址，或粘贴 ObsidianDragon 完整导出');
}
